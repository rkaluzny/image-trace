...
FORENSIC_TAGS = [
    ...
    "EXIF FNumber", "EXIF ISOSpeedRatings", "EXIF ExposureTime", "Image Artist",
...
if PIEXIF_AVAILABLE:
    TAG_MAP = {
        ...
        "Image Copyright": ("0th", piexif.ImageIFD.Copyright),
...
try:
    with open("plausible_data.json", "r") as f:
        _plausible_data = json.load(f)
    PLOSIBLE_MAKES = _plausible_data.get("makes", [])
    PLOSIBLE_MODELS = _plausible_data.get("models", {})
    PLOSIBLE_SOFTWARE = _plausible_data.get("software", [])
    PLOSIBLE_ARTISTS = _plausible_data.get("artists", [])
    PLOSIBLE_F_NUMBERS = _plausible_data.get("f_numbers", [])
    PLOSIBLE_EXPOSURE_TIMES = _plausible_data.get("exposure_times", [])
    PLOSIBLE_ISO_SPEEDS = _plausible_data.get("iso_speeds", [])
except (FileNotFoundError, json.JSONDecodeError):
    # Fallback to empty lists/dicts if file is missing or corrupt
    PLOSIBLE_MAKES, PLOSIBLE_MODELS, PLOSIBLE_SOFTWARE, PLOSIBLE_ARTISTS = [], {}, [], []
    PLOSIBLE_F_NUMBERS, PLOSIBLE_EXPOSURE_TIMES, PLOSIBLE_ISO_SPEEDS = [], [], []
...
def _extract_dqt_hash(data):
    ...
    try:
        ...
        # Ensure that quantization tables are loaded
        img.load()
        if img.format == 'JPEG' and hasattr(img, 'quantization'):
            dqt_bytes = b''
            # Sort by table index for a consistent hash
            for table_index in sorted(img.quantization.keys()):
                table = img.quantization[table_index]
                for val in table:
                    # DQT values can be > 255, so use 2 bytes
                    dqt_bytes += val.to_bytes(2, 'big')
            if dqt_bytes:
...
def generate_trustworthy_exif(exif_dict):
    ...
    make = random.choice(PLOSIBLE_MAKES) if PLOSIBLE_MAKES else "Unknown Make"
    
    # Ensure model is compatible with selected make, or pick a generic one
    if make in PLOSIBLE_MODELS and PLOSIBLE_MODELS[make]:
        model = random.choice(PLOSIBLE_MODELS[make])
    else:
        model = "Digital Camera" # Fallback
        
    software = random.choice(PLOSIBLE_SOFTWARE) if PLOSIBLE_SOFTWARE else "Unknown Software"
    artist = random.choice(PLOSIBLE_ARTISTS) if PLOSIBLE_ARTISTS else "Unknown Artist"
    
    # Add new plausible photographic parameters
    f_number = tuple(random.choice(PLOSIBLE_F_NUMBERS)) if PLOSIBLE_F_NUMBERS else (28, 10) # f/2.8 as default
    exposure_time = tuple(random.choice(PLOSIBLE_EXPOSURE_TIMES)) if PLOSIBLE_EXPOSURE_TIMES else (1, 125) # 1/125s as default
    iso = random.choice(PLOSIBLE_ISO_SPEEDS) if PLOSIBLE_ISO_SPEEDS else 200
...
# Load the DQT database from the external JSON file.
try:
    with open("dqt_database.json", "r") as f:
        KNOWN_DQT_HASHES = json.load(f)
except (FileNotFoundError, json.JSONDecodeError):
    KNOWN_DQT_HASHES = {} # Fallback to empty dict if file is missing or corrupt
...
def fingerprint_software(meta):
    ...
    # First, check for DQT hash if available and if the database was loaded
    if KNOWN_DQT_HASHES and "JPEG DQT Hash" in meta:
        dqt_hash = meta["JPEG DQT Hash"]
        if dqt_hash in KNOWN_DQT_HASHES:
            match = KNOWN_DQT_HASHES[dqt_hash]
            source = match.get("source", "Unknown Source")
            version = match.get("version", "")
            quality = match.get("quality", "")
            details = f"{source} {version} (Quality: {quality})".strip()
            return f"Identified by DQT Hash: {details}"
        else:
...
def perform_ela(data, quality=90):
    ...
    try:
        ...
        ela_image_bytes = ela_buffer.getvalue()

        return ela_image_bytes, "ELA performed successfully."
    except Exception as e:
...
def export_html(results, output_path, all_tags=False):
    ...
    for r in results:
        ...
        # New ELA report section
        ela_report = ""
        if "ela_image_bytes" in r:
            ela_base64 = base64.b64encode(r['ela_image_bytes']).decode('ascii')
            ela_report = f"""
            <h4>Error Level Analysis (ELA)</h4>
            <div class="ela-container">
                <img src="data:image/png;base64,{ela_base64}" alt="ELA Image" style="max-width:100%;">
            </div>
            """
