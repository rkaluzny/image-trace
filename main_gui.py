...
class ModifyWindow(Toplevel):
    def __init__(self, master):
        ...
        self.geometry("650x700") # Increased size for new options
    ...
    def build_ui(self):
        ...
        # Sort tags alphabetically for a consistent order
        editable_tags = sorted(list(core.TAG_MAP.keys()))
        for i, tag in enumerate(editable_tags):
            Label(scrollable_frame, text=tag, width=25, anchor="w").grid(row=i, column=0, sticky="w", padx=5, pady=2)
    ...
    def _update_state(self):
        ...
        is_set_action = (selected == "set")

        # Define styles for enabled/readonly states for better visual feedback
        enabled_bg = "white"
        readonly_bg = "#f0f0f0"  # A standard light grey, looks disabled

        for entry in self.tag_entries.values():
            # To clear a readonly entry, it must be temporarily made normal
            if not is_set_action:
                entry.config(state="normal")
                entry.delete(0, "end")

            if is_set_action:
                entry.config(state="normal", bg=enabled_bg)
            else:
                # Readonly state prevents user edits but allows background color changes
                entry.config(state="readonly", readonlybackground=readonly_bg)
